...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  double position_L1_x = (-settings.x_distance/2) - position[X_AXIS]/settings.steps_per_mm[X_AXIS];
  double position_L1_y = (settings.y_distance/2) - position[Y_AXIS]/settings.steps_per_mm[Y_AXIS];
  double position_L1 = sqrt(square(position_L1_x) + square(position_L1_y));

  double position_L2_x = (settings.x_distance/2) - position[X_AXIS]/settings.steps_per_mm[X_AXIS];
  double position_L2_y = (settings.y_distance/2) - position[Y_AXIS]/settings.steps_per_mm[Y_AXIS];
  double position_L2 = sqrt(square(position_L2_x) + square(position_L2_y));

  double position_L1_steps = position_L1 * settings.steps_per_mm[Y_AXIS];
  double position_L2_steps = position_L2 * settings.steps_per_mm[X_AXIS];

  double target_L1_x = (-settings.x_distance/2) - target[X_AXIS]/settings.steps_per_mm[X_AXIS];
  double target_L1_y = (settings.y_distance/2) - target[Y_AXIS]/settings.steps_per_mm[Y_AXIS];
  double target_L1 = sqrt(square(target_L1_x) + square(target_L1_y));

  double target_L2_x = (settings.x_distance/2) - target[X_AXIS]/settings.steps_per_mm[X_AXIS];
  double target_L2_y = (settings.y_distance/2) - target[Y_AXIS]/settings.steps_per_mm[Y_AXIS];
  double target_L2 = sqrt(square(target_L2_x) + square(target_L2_y));

  double target_L1_steps = target_L1 * settings.steps_per_mm[Y_AXIS];
  double target_L2_steps = target_L2 * settings.steps_per_mm[X_AXIS];
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  */
  ...
  // New Version
  block->steps_x = labs(target_L2_steps - position_L2_steps);
  block->steps_y = labs(target_L1_steps - position_L1_steps);
  block->steps_z = labs(target[Z_AXIS]-position[Z_AXIS]);
  block->step_event_count = max(block->steps_x, max(block->steps_y, block->steps_z));
...
/*
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
//This is where the transformation will be put based on the geometry of the work surface. It takes the values
// of delta_x_mm and delta_y_mm and makes it so that they are corrected based on their absolute location. 

  double position_L1 = hypot(((-settings.x_distance/2) - (position[X_AXIS]/settings.steps_per_mm[X_AXIS])), ((settings.y_distance/2) - (position[Y_AXIS]/settings.steps_per_mm[Y_AXIS]))); 
  double position_L2 = hypot(((settings.x_distance/2) - (position[X_AXIS]/settings.steps_per_mm[X_AXIS])), ((settings.y_distance/2) - (position[Y_AXIS]/settings.steps_per_mm[Y_AXIS])));

  double target_L1 = hypot(((-settings.x_distance/2) - (target[X_AXIS]/settings.steps_per_mm[X_AXIS])), ((settings.y_distance/2) - (target[Y_AXIS]/settings.steps_per_mm[Y_AXIS]))); 
  double target_L2 = hypot(((settings.x_distance/2) - (target[X_AXIS]/settings.steps_per_mm[X_AXIS])), ((settings.y_distance/2) - (target[Y_AXIS]/settings.steps_per_mm[Y_AXIS])));

  double delta_L1 = target_L1 - position_L1;
  double delta_L2 = target_L2 - position_L2;
  */

  delta_x_mm = (target_L2_steps - position_L2_steps)/settings.steps_per_mm[X_AXIS];
  delta_y_mm = (target_L1_steps - position_L1_steps)/settings.steps_per_mm[Y_AXIS];
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  */
	...
	// L2 is coiled in the opposite direction so it has the opposite direction bit.
	 
  if (target_L2 < position_L2) { block->direction_bits |= (1<<X_DIRECTION_BIT); } 
  if (target_L1 > position_L1) { block->direction_bits |= (1<<Y_DIRECTION_BIT); }
  if (target[Z_AXIS] < position[Z_AXIS]) { block->direction_bits |= (1<<Z_DIRECTION_BIT); }
